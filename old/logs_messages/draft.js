...
const channel_logs = "LOG CHANNEL ID HERE"; // logs channel id

client.on("message", message => {

	function send_in_channel_logs(msg) {
		if (message.guild.channels.has(channel_logs)) {
			message.guild.channels.get(channel_logs).send({ embed: msg}).then().catch(err => console.log(err));
		}
	}
	...
	if (entry_links > banLevel.links) {
		message.member.ban(1).then(member => {
			...
			send_in_channel_logs(new Discord.RichEmbed().setTitle(':hammer: Banned').setColor(0xFF0000).setTimestamp().addField('User', `${message.author.tag} (${message.author.id})`).addField('Reason', `Posting too many links (${entry_links}x)`));
			slowmode_links.delete(message.author.id);
		})
		.catch(e => {
			send_in_channel_logs(new Discord.RichEmbed().setTitle(':x: ERROR').setColor(0x000001).setTimestamp().addField('User', `${message.author.tag} (${message.author.id})`).addField('Reason', `Could not ban because they have a higher role`));
		});
	} else {
		setTimeout(()=> {
			entry_links -= message.embeds.length;
	...
	if (entry_mentions > banLevel.mentions) {
		message.member.ban(1).then(member => {
			...
			send_in_channel_logs(new Discord.RichEmbed().setTitle(':hammer: Banned').setColor(0xFF0000).setTimestamp().addField('User', `${message.author.tag} (${message.author.id})`).addField('Reason', `Mentioning too many users (${entry_mentions}x)`));
	...
	if (entry_attachments > banLevel.attachments) {
		message.member.ban(1).then(member => {
			...
			send_in_channel_logs(new Discord.RichEmbed().setTitle(':hammer: Banned').setColor(0xFF0000).setTimestamp().addField('User', `${message.author.tag} (${message.author.id})`).addField('Reason', `Posting too many images (${entry_attachments}x)`));
